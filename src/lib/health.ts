...
import { getHasuraBaseUrl } from './hasura';
...
async function pingHasura(): Promise<ServiceCheck> {
  let baseUrl: string;
  try {
    baseUrl = getHasuraBaseUrl();
  } catch {
    return { status: 'error', error: 'HASURA_GRAPHQL_URL or NHOST_HASURA_URL not configured' };
  }
...
export async function checkHealth(): Promise<HealthResult> {
  const [hasura, redisCheck, s3] = await Promise.all([pingHasura(), pingRedis(), pingS3()]);

  const allOk = hasura.status === 'ok' && redisCheck.status === 'ok' && s3.status === 'ok';

  return {
    status: allOk ? 'healthy' : 'degraded',
    uptime: Math.floor(process.uptime()),
    checks: { hasura, redis: redisCheck, s3 },
  };
}
