...
} from '../../../../lib/graphql/restaurant-queries';
  ...
  try {
    ...
    if (place_id) {
      ...
      if (result.data?.restaurants?.length > 0) {
        res.json({ match: true, restaurant: result.data.restaurants[0], matchType: 'place_id' });
        return;
      }
    }
    ...
    if (name && address) {
      ...
      if (result.data?.restaurants?.length > 0) {
        res.json({ match: true, restaurant: result.data.restaurants[0], matchType: 'name_address' });
        return;
      }
    }
    ...
    if (latitude && longitude) {
      ...
      if (result.data?.restaurants?.length > 0) {
        ...
        if (closest) {
          res.json({ match: true, restaurant: closest, matchType: 'coordinates' });
          return;
        }
      }
    }
