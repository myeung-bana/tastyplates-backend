...
<head>
  ...
  <title>Tastyplates Backend — Health</title>
  <style>
...
<body>
  <h1>Tastyplates Backend</h1>
  <p class="subtitle">Health monitor — refreshes every 10 s</p>
    ...
    async function refresh() {
      try {
        ...
        document.getElementById('uptime').textContent =
          'Uptime: ' + d.uptime + 's · Status: ' + (d.status || '—');
      } catch (e) {
        document.getElementById('uptime').textContent = 'Fetch error: ' + e.message;
      }
    }
    refresh();
